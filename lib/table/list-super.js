...
const ListSuper = ({
  ...
  useEffect(() => {
    ...
    const numberOfTotalRows2 = data2 ? data2.length : 0;
    dispatch({
      type: ActionType.FETCH_DATA_SUCCESS,
      payload: {
        data: data2,
        numberOfTotalRows: numberOfTotalRows2
      }
    });
  }, [props.data]);
  ...
  const renderBody = (data2) => {
    return data2.map((row, i) => /* @__PURE__ */ React.createElement(React.Fragment, {
      key: i
